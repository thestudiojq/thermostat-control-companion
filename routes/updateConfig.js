import { readConfig, definedIp, definedApiKey, definedInterval, areNotificationsEnabled, isRemoteRetrieveEnabled } from '../readConfig.js'
...
const saveConfigData = async function ( req, res ) {
	...
	if ( ipAddress != '' && apiKey != '' && interval != '' ) {
		...
		const newData = { ip: ipAddress, apikey: apiKey, interval: interval, notificationsenabled: notificationsEnabled, remoteretrieve: remoteRetrieve  };
...
const populateConfigData = async function ( req, res ) {
	...
	await readConfig.init();
	res.status( 200 ).json( { ip: definedIp, interval: definedInterval, notificationsenabled: areNotificationsEnabled, remoteretrieve: isRemoteRetrieveEnabled } );
}
